...
const EditBook = ({ data, isModal, onClose }) => {
  const fileInputRef = useRef(null);

  if (!isModal) return null;
  
  const user = useSelector((state) => state.userdata.user_data);
  const { Update_BookAxios} = useBooks();
  ...
  // Initial form values from the provided data
  const initialValues = {
  ...
  return (
    <div className="fixed inset-0 bg-whilte/10 backdrop-blur-sm bg-opacity-50 flex items-center overflow-auto pt-52 pb-4 px-6  rounded-2xl justify-center z-50" onClick={onClose}>
      <div
